    ...
    def prepare(self, filename):
        ...
        while cap.isOpened():
            ...
            if ret:
                ...
                # we force it to 256 for kp detection
...
def get_others(root, video_name, data_type):
    ...
    if os.path.isfile(os.path.join(root, 'processed', video_name+'.mat')):
        file_path = os.path.join(root, 'processed', video_name+'.mat')
    else:
        file_path = os.path.join(root, 'processed', video_name+'.mat')
...
def prepare_data(path, keypoint_path, coeff_3dmm_path, out, n_worker, sizes, chunksize, img_format):
    ...
    filenames = glob.glob(os.path.join(coeff_3dmm_path, 'processed', '*.mat'))
    filenames = [fn.replace('.mat', '.mp4').replace('coef/','') for fn in filenames]
    # import pdb; pdb.set_trace()
    ...
    for size in sizes:
        ...
        with lmdb.open(lmdb_path, map_size=1024 ** 4, readahead=False) as env:
            with env.begin(write=True) as txn:
                ...
                # idxes, results, filenames = resizer(filenames)
                # for idx, result, filename in tqdm(zip(idxes, results, filenames)):
                for jj, filename in tqdm(enumerate(filenames)):
                    idx, result, filename = resizer([jj, filename])
