...
def create_high_intensity_emotions(talking_head, sample, identity_list=None, 
    ...
    for emo_idx, int_idx, identity_idx in zip(emotion_index_list, intensity_list, identity_list):
        # # for int_idx in range(0, talking_head.get_num_intensities()):
...
def run_evalutation(talking_head, 
    ...
    for bi in range(1):
        ...
        with torch.no_grad():
            if is_external_style_emb is True:
                batch = talking_head(batch, style_emb=style_emb, is_external_style_emb=is_external_style_emb)
            else:
                batch = talking_head(batch)

        # import pdb; pdb.set_trace()
        if gt_exp is not None or gt_pose is not None:
            timestep = batch['predicted_exp'].shape[1]

            batch['predicted_exp'], batch['predicted_jaw'] = torch.from_numpy(gt_exp).unsqueeze(0)[:,:timestep], torch.from_numpy(gt_pose).unsqueeze(0)[:,:timestep,3:]
            pose = torch.cat([torch.zeros_like(batch['predicted_jaw']), batch['predicted_jaw']], dim=-1) 
            exp = batch["predicted_exp"]
            ...
            predicted_verts, _, _ = flame(shape, exp.to(shape.device), pose.to(shape.device))
            predicted_verts = predicted_verts.reshape(B_, T_, -1)
            batch["predicted_vertices"] = predicted_verts
            # import pdb; pdb.set_trace()
