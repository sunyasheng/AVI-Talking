...
class FeedForwardDecoder(nn.Module): 
    ...
    def forward(self, sample, style_emb=None, train=False, teacher_forcing=True, only_style_emb=False, is_external_style_emb=False):
        if only_style_emb:
            styled_states = self._style_emb(sample)
            return styled_states
        else: 
            sample["hidden_feature"] = sample["seq_encoder_output"]
            hidden_states = self._positional_enc(sample)
            # import pdb; pdb.set_trace();
            styled_hidden_states = self._style(sample, hidden_states, style_emb=style_emb, is_external_style_emb=is_external_style_emb)
    ...
    def _style(self, sample, hidden_states, style_emb=None, is_external_style_emb=False): 
        if style_emb is not None and is_external_style_emb is True:
            pass
        else:
            if self.obj_vector is None:
                return hidden_states
...
class BertPriorDecoder(FeedForwardDecoder):
    ...
    def forward(self, sample, train=False, teacher_forcing=True, only_style_emb=False, style_emb=None, is_external_style_emb=False): 
        sample = super().forward(sample, train=train, teacher_forcing=teacher_forcing, only_style_emb=only_style_emb, style_emb=style_emb, is_external_style_emb=is_external_style_emb)
        return sample
