...
class TalkingHeadBase(pl.LightningModule): 
    ...
    def _choose_primary_3D_rec_method(self, batch): 
        ...
        if "reconstruction" in batch:
            for k in batch["reconstruction"][method].keys():
                ...
                if k in batch.keys():
                    print(f"Key '{k}' already exists in batch. We don't want to overwrite it. -- TalkingHeadBased.py")
    ...
    def decode_sequence(self, sample: Dict, style_emb=None, train=False, teacher_forcing=False, only_style_emb=False, is_external_style_emb=False, **kwargs: Any) -> Dict:
        return self.sequence_decoder(sample, style_emb=style_emb, train=train, teacher_forcing=teacher_forcing, only_style_emb=only_style_emb, is_external_style_emb=is_external_style_emb, **kwargs)
    ...
    def forward(self, sample: Dict, style_emb=None, train=False, validation=False, only_style_emb=False, is_external_style_emb=False, **kwargs: Any) -> Dict:
        """
        ...
        # decode the sequence
        sample = self.decode_sequence(sample, style_emb=style_emb, train=train, teacher_forcing=teacher_forcing, only_style_emb=only_style_emb, is_external_style_emb=is_external_style_emb, **kwargs)
        if only_style_emb:
            return sample
        else:
            check_nan(sample)
