...
class Faceformer(nn.Module):
    def __init__(self, args):
        ...
        if args.w_fan == 1:
            ...
            self.coeff2style = nn.Linear(self.coeff_mean.shape[-1], 6)
    ...
    def forward_switch_frame(self, audio, coeff_b, pose, shape, cam=None, motion_des=None, img=None, ref_img=None, criterion=None,
        ...
        ref_coeff = torch.index_select(coeff_b, 1, random_indices)
        ref_style_embed = self.coeff2style(ref_coeff)
        ref_style_embed = ref_style_embed.repeat(1,frame_num,1)
        # print(coeff_b.shape, ref_style_embed.shape)
        # import pdb; pdb.set_trace();
        
        hidden_states = torch.cat([eye_embed, emo_wolip_embed, hidden_states_a, ref_style_embed], dim=-1)
        ...
        for j in range(len(hidden_states_mix)):
            ...
            if teacher_forcing is True:
                coeff = coeffs[j:j + 1]
                # vertice_emb = self.obj_embedding.unsqueeze(1)
                # style_emb = vertice_emb
                vertice_input = torch.cat([torch.zeros_like(coeff[:, -1:]), coeff[:, :-1]], 1)
                ...
                for i in range(frame_num):
                    if i == 0:
                        vertice_emb = self.obj_embedding.unsqueeze(1)  # (1,1,feature_dim)
                        style_emb = vertice_emb
                        vertice_input = self.PPE(style_emb)
                    else:
                        vertice_input = self.PPE(vertice_emb)
                    tgt_mask = self.biased_mask[:, :vertice_input.shape[1],
                               :vertice_input.shape[1]].clone().detach().to(device=self.device)
                    memory_mask = enc_dec_mask(self.device, self.dataset, vertice_input.shape[1],
        ...
        loss = criterion(vertice_outs[:,:,:53], coeffs[:,:,:53])*self.args.lip_coeff_weight  # (batch, seq_len, V*3)
        loss = torch.mean(loss)
        loss_coeff_dict = {'coeff': loss}
    ...
    def predict(self, audio, head_img, eye_img, emotion_img, text=None):
        self.coeff_mean, self.coeff_std = self.coeff_mean.to(audio), self.coeff_std.to(audio)
        ...
        hidden_states_all = torch.cat([eye_embed, emo_embed, hidden_states_a, head_embed], dim=-1)
        hidden_states = self.v_merge2hidden(hidden_states_all)
        ...
        # import pdb; pdb.set_trace()
        vertice_out = vertice_out*self.coeff_std + self.coeff_mean
        return vertice_out


    # def forward_switch_batch(self, audio, coeff, pose, shape, cam=None, motion_des=None, img=None, ref_img=None, criterion=None,
    #             file_name=None, text_desc=None, teacher_forcing=True):
    #     emo_label = torch.tensor([self.emo2idx[fn.split('_')[2]] for fn in file_name])
    #     emo_label = emo_label.to(audio.device)
    #
    #     frame_num = coeff.shape[1]
    #     hidden_states_a = self.audio_encoder(audio, self.dataset, frame_num=frame_num).last_hidden_state
    #     hidden_states_a = self.audio_feature_map(hidden_states_a)
    #
    #     with torch.no_grad():
    ...
    #         head_embed = torch.stack(head_embed, dim=0)
    #
    #         upper_face_v00 = torch.cat([eye_embed, emo_embed], dim=-1)
    #         upper_face_v11 = torch.stack([upper_face_v00[1], upper_face_v00[0]], dim=0)
    #         eye_embed_reverse = torch.stack([eye_embed[1], eye_embed[0]], dim=0)
    #         upper_face_v01 = torch.cat([eye_embed_reverse, emo_embed], dim=-1)
    #         emo_embed_reverse = torch.stack([emo_embed[1], emo_embed[0]], dim=0)
    #         upper_face_v10 = torch.cat([eye_embed, emo_embed_reverse], dim=-1)
    #
    #     upper_faces_v00 = upper_face_v00.detach().requires_grad_(True)
    #     upper_faces_v11 = upper_face_v11.detach().requires_grad_(True)
    #     upper_faces_v01 = upper_face_v01.detach().requires_grad_(True)
    #     upper_faces_v10 = upper_face_v10.detach().requires_grad_(True)
    #     head_embed = head_embed.detach().requires_grad_(True)
    #
    #     hidden_states_000 = torch.cat([upper_faces_v00, hidden_states_a, head_embed], dim=-1)
    #     hidden_states_011 = torch.cat([upper_faces_v11, hidden_states_a, head_embed], dim=-1)
    #     hidden_states_001 = torch.cat([upper_faces_v01, hidden_states_a, head_embed], dim=-1)
    #     hidden_states_010 = torch.cat([upper_faces_v10, hidden_states_a, head_embed], dim=-1)
    #
    #     hidden_states_all_mix = torch.cat([hidden_states_000, hidden_states_011,
    #                                        hidden_states_001, hidden_states_010], dim=0)
    #     hidden_states_mix = self.v_merge2hidden(hidden_states_all_mix)
    #
    #     coeffs = torch.cat([coeff,]*4)
    #     vertice_outs = []
    #     for j in range(len(hidden_states_mix)):
    #         hidden_states = hidden_states_mix[j:j + 1]
    #         if teacher_forcing is True:
    ...
    #
    #     vertice_outs = torch.concat(vertice_outs)
    #     loss = criterion(vertice_outs[:2, :, :53], coeffs[:2, :, :53]) # (batch, seq_len, V*3)
    #     loss = torch.mean(loss)
    #     # print('base loss: ', loss.item())
    #     if self.args.w_lip_ldmk_loss:
    #         poses, shapes, cams = torch.cat([pose,]*4), torch.cat([shape,]*4), torch.cat([cam,]*4)
    #         # print(vertice_outs.shape, coeffs.shape)
    #         # import pdb; pdb.set_trace()
    #         loss_lip_ldmk, lip_ldmk_dict = self.compute_ldmk_loss(vertice_outs[:,:,:53],
    #                                                               coeffs[:,:,:53],
    #                                                               poses, shapes, gt_cam=cams,
    ...
    #         loss = torch.mean(loss_lip_ldmk) * 10. + loss
    #
    #     if self.args.w_eye_ldmk_loss:
    #         poses, shapes, cams = torch.cat([pose,]*4), torch.cat([shape,]*4), torch.cat([cam,]*4)
    #         # import pdb; pdb.set_trace()
    #         # poses, shapes, cams = pose, shape, cam
    #         coeffs_reshuffle = torch.stack([coeffs[0],coeffs[1],
    #                                         coeffs[1],coeffs[0],
    #                                         coeffs[1],coeffs[0],
    #                                         coeffs[0],coeffs[1]])
    #         loss_eye_ldmk, eye_ldmk_dict = self.compute_ldmk_loss(vertice_outs[:,:,:53],
    #                                                               coeffs_reshuffle[:,:,:53],
    #                                                               poses, shapes, gt_cam=cams,
    #                                                               lmk_weight=0, lipd_weight=0,
    #                                                               eyed_weight=1)
    #         # print('loss ldmk: ', gt_ldmk_dict)
    #         # print(loss, loss_ldmk)
    #         # print(eye_ldmk_dict)
    #         loss = torch.mean(loss_eye_ldmk) * 10. + loss
    #     # import pdb; pdb.set_trace()
    #
    #     if self.args.w_render_loss or self.args.w_emo_loss:
    #         poses, shapes, cams, motion_dess, imgs, ref_imgs = \
    #                 torch.cat([pose,]*4), torch.cat([shape,]*4), torch.cat([cam,]*4), \
    #                     torch.cat([motion_des,]*4), torch.cat([img,]*4), torch.cat([ref_img,]*4)
    #         # import pdb; pdb.set_trace()
    #         select_input_image, select_ref_image, select_emo_image, output = self.render2image(vertice_outs, coeffs, poses, shapes, cams, motion_dess, imgs, ref_imgs)
    #
    #     if self.args.w_render_loss:
    #         # loss_render, render_dict = self.compute_render_loss(vertice_outs[:2], coeffs[:2], poses, shapes, cams, motion_des, img)
    #         # loss = torch.mean(loss_render) * 10. + loss
    #         pass
    #
    #     # import pdb; pdb.set_trace()
    #     if self.args.w_emo_loss:
    #         batch_size = poses.shape[0]
    #         emo_labels = torch.cat([emo_label,]*4)
    #         loss_emo, loss_emo_dict = self.compute_emo_loss(gt_image=select_input_image, ref_image=select_ref_image,
    #                                                         emo_labels=emo_labels, output_dict=output, batch_size=batch_size)
    #         # 0.0015, official weight
    #         # print('loss_emo: ', loss_emo.item())
    #         # print('emo dict: ', loss_emo_dict)
    #         loss = loss_emo * 0.015 + loss
    #
    #     return loss

    # def compute_switch_emo_loss(self, gt_image, ref_image, emo_labels, output_dict, batch_size=8):
    #     loss_dict = {}
    #
    #     # gt_image_unfold = gt_image.view(batch_size, -1, *gt_image.shape[1:])
    #     # pi_key = 'warp_image'
    #     pi_key = 'fake_image'
    #     pred_image_unfold = output_dict[pi_key].view(batch_size, -1, *output_dict[pi_key].shape[1:])
    #     # 1 pos 2 neg 3 pos 4 neg
    #     pred_image_unfold_flip = [pred_image_unfold[0], pred_image_unfold[1],
    #                               pred_image_unfold[3], pred_image_unfold[2],
    #                               pred_image_unfold[4], pred_image_unfold[5],
    #                               pred_image_unfold[7], pred_image_unfold[6]]
    #     pred_image_unfold_flip = torch.stack(pred_image_unfold_flip)
    #     pred_image_unfold_flip = pred_image_unfold_flip.view(-1, *pred_image_unfold_flip.shape[2:])
    #
    #     # self.vis_image = torch.cat([gt_image,pred_image_unfold_flip,output_dict['warp_image']],dim=2)*0.5+0.5
    #     self.vis_dict = {}
    #     self.vis_dict['gt_image'] = gt_image*0.5 + 0.5
    #     self.vis_dict['pred_image_unfold_flip'] = pred_image_unfold_flip*0.5 + 0.5
    #     self.vis_dict[pi_key] = output_dict[pi_key]*0.5 + 0.5
    #     self.vis_dict['ref_image'] = ref_image*0.5 + 0.5
    #
    #     loss_emo = self.deca_emo_loss(pred_image_unfold_flip*0.5+0.5, gt_image*0.5+0.5)
    #     ## when they are the same, no need to deal
    #     if emo_labels[0] == emo_labels[1]: loss_emo = loss_emo * 0.0
    #     loss_ecls = self.emo_class_loss(pred_image_unfold_flip*0.5+0.5, emo_labels, batch_size=batch_size)
    #
    #     # import torchvision
    #     # torchvision.utils.save_image(gt_image*0.5+0.5, 'gt_image.jpg')
    #     # torchvision.utils.save_image(output_dict['warp_image']*0.5+0.5, 'pred_warp.jpg')
    #     # torchvision.utils.save_image(pred_image_unfold_flip*0.5+0.5, 'rearrange_warp.jpg')
    #     # import pdb; pdb.set_trace()
    #
    #     loss_dict['emo'] = loss_emo
    #     loss_dict['ecls'] = loss_ecls * 0.5
