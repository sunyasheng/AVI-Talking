import torch
from .BaseModel import BaseReconModel
...
class BFM09ReconModel(BaseReconModel):
    ...
    def forward(self, coeffs, render=True):
        ...
        if render:
            ...
            gray_texture = torch.ones_like(face_texture)*127.0
            gray_color = self.add_illumination(gray_texture, face_norm_r, gamma)
            gray_color_tv = TexturesVertex(gray_color)
            mesh_gray = Meshes(vs_t, self.tri.repeat(batch_num, 1, 1), gray_color_tv)
            rendered_img_gray = self.renderer(mesh_gray)

            rendered_img = torch.clamp(rendered_img, 0, 255)

            return {'rendered_img': rendered_img,
                    'rendered_img_gray': rendered_img_gray,
                    'lms_proj': lms_proj,
